...
import { COMMISSIONS } from '../../constants/index.js';
...
const Commissions = () => {
  ...
  const setupCommission = (commission) => {
    setInfo(
      <>
    ...
    setContent(
      <>
        <h1 className='pt-[1%] tracking-widest font-black text-center text-2xl text-red-600 '>
          {commission.name}
          <span className='absolute -translate-x-[99.95%] translate-y-[1%] font-semibold text-gray-300'> {commission.name} </span>
          <span className='absolute -translate-x-[99%] translate-y-[2%] font-semibold text-black'> {commission.name} </span>
        </h1>
        ...
        <div className='relative scale-[100%] -translate-y-[0%]'>
          <div className="flex justify-center">
            <ReactPlayer url={commission.media} />
          </div>
        </div>
      </>
    );
  ...
  return (
    <div>
      ...
      <Parallax ref={parallaxRef} pages={COMMISSIONS.length} className='bg-[url(https://w.wallhaven.cc/full/nm/wallhaven-nm9omy.jpg)]'>
        {COMMISSIONS.map((commission, index) => (
          <ParallaxLayer key={index} offset={index} factor={1} speed={.1} id={`commission-${index}`}>
            <div className='relative flex justify-center h-screen'>
              ...
              <div className='fixed origin-top left-[30%] top-[40%] w-[40%] h-[25%]'
                onClick={() => setupCommission(commission)}
